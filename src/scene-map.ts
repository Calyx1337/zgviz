...
export class SceneMap {
 ...
 private listeners=new Map<string,((e:any)=>void)[]>();private layers=new Map<string,{group:T.Group;data:any;kind:string;color:string;visible:boolean;opacity:number;width:number;lineOpacity:number}>();
 ...
 addLayer(l:any){this.layers.set(l.id,{group:new T.Group(),data:l.source,kind:l.type,color:(l.type==='line'?l.paint?.['line-color']:l.paint?.['fill-color'])||'#6895f7',visible:l.layout?.visibility!=='none',opacity:l.paint?.['fill-opacity']??.13,width:l.paint?.['line-width']??0,lineOpacity:l.paint?.['line-opacity']??1});this.draw(l.id);}
 ...
 private draw(id:string){const l=this.layers.get(id)!;this.scene.remove(l.group);dispose(l.group);l.group=new T.Group();l.group.visible=l.visible;l.group.renderOrder=6;const data=this.sourceData.get(l.data);const features=data?.type==='FeatureCollection'?data.features:data?.type==='Feature'?[data]:[];
  for(const f of features){const g=f.geometry;if(!g)continue;let color=id==='extra-landuse'?f.properties.displayColor||l.color:l.color;if(id==='district-fill'){const v=this.investment?f.properties.plannedValue/5000000:f.properties.accessDisplay/100;color=this.investment&&!f.properties.projects?'#d8dde3':'#'+new T.Color('#d9e6f6').lerp(new T.Color('#2457c8'),Math.min(1,v||0)).getHexString();}
   ...
   if(l.kind==='line'){const vertices:number[]=[];for(const line of lines)for(let i=1;i<line.length;i++){const a=this.position(line[i-1]),b=this.position(line[i]),n=Math.max(1,Math.ceil(a.distanceTo(b)/30));for(let j=0;j<n;j++){for(const k of [j/n,(j+1)/n]){const p=a.clone().lerp(b,k);p.y=this.terrain.heightAt(p.x,-p.z)+2;vertices.push(p.x,p.y,p.z);}}}const geo=new T.BufferGeometry();geo.setAttribute('position',new T.Float32BufferAttribute(vertices,3));let mesh:T.Mesh|T.LineSegments;
    // Line widths are metres on the ground. Widths of 3 m and more become draped ribbons; narrower ones stay 1 px hairlines.
    const half=id==='extra-closures'||id==='extra-selected'?2:l.width>=3?l.width/2:0;
    if(half){const triangles:number[]=[];for(let i=0;i<vertices.length;i+=6){const a=new T.Vector3(...vertices.slice(i,i+3) as [number,number,number]),b=new T.Vector3(...vertices.slice(i+3,i+6) as [number,number,number]);const normal=new T.Vector3(-(b.z-a.z),0,b.x-a.x).normalize().multiplyScalar(half);for(const p of [a.clone().add(normal),a.clone().sub(normal),b.clone().add(normal),b.clone().add(normal),a.clone().sub(normal),b.clone().sub(normal)])triangles.push(p.x,p.y,p.z);}geo.setAttribute('position',new T.Float32BufferAttribute(triangles,3));mesh=new T.Mesh(geo,new T.MeshBasicMaterial({color,side:T.DoubleSide,transparent:l.lineOpacity<1,opacity:l.lineOpacity,depthWrite:false}));}
